...
module ErrorHandler
  ...
  included do
    ...
    rescue_from ErrorHandler::MissingSession, with: :unauthorized_request
        ...
        json: {
          ...
          message:
              I18n.t("activerecord.errors.messages.#{err.model.underscore}.#{key}",
                     default: I18n.t("activerecord.errors.messages.#{key}", model: I18n.t("activerecord.models.#{err.model.underscore}"), id: err.id))
        },
        status: :not_found
      )
    end
    ...
    rescue_from RGeo::Error::InvalidGeometry do |_err|
      render(
        json: { success: false, message: I18n.t(:invalid_polygon) },
        status: :bad_request
      )
    end
  end
